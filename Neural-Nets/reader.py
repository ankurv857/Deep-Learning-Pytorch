...
class data_read():
    def __init__(self,dir ,dataframe_list  ,date_list , target_list ,idx, multiclass_discontinuous , text, remove_list):
        ...
        self._init_data_check_(self.df_list , self.dtype_list , self.dtype_list_name)

    def _init_read_(self,dataframes):
        df_list = []
        for df in dataframes:
            data = pd.read_csv(os.path.join(self.dir ,df) , na_values = ' ', low_memory=False).head(100000)
            print(data.columns)
            df_list.append(data)
        return df_list
